...
1. 프로젝트가 커지면서 서비스 계층의 역할이 너무 커진다.
- 기존의 서비스 계층의 역할을 다시 한번 생각해보자.
  - 애플리케이션 내부 사용자가 필요한 데이터를 리턴 (DTO)
  - 저장소에 데이터를 삽입, 수정, 삭제를 진행하기 위해서 필요한 행위를 진행
  - 입력 모델에 대한 유효성 검사
  - 비지니스 로직에 대한 유효성 검사
  - 등등..
- 비지니스 로직에 필요한 역할을 서비스에서 거의 다 처리한다.
- 프로젝트 규모가 작다면 계층형 아키텍처가 좋은 강점을 가져갈 수 있지만 변경이 잦아지고 프로젝트의 규모가 커질 수록 ***새로운 개발보다 서비스 로직을 파악하는데 더욱 많은 시간을 쓰는일이 발생***한다.
  - 아마 회사에서 개발을 진행해보신 분이라면 어느정도 공감을 할 수 있을 것이다.
