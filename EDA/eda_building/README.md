...
- 또한 이벤트 발행이 성공적으로 이루어지고, 해당 이벤트를 소비하는 애플리케이션이 정상적으로 처리되었다고 하더라도 이벤트 처리를 잘못하고 있을 수도 있기 때문에 특정 시점에 대한 이벤트를 재발행해야할 수도 있다.
- 위와 같은 이유 때문에 이벤트 발행 시점에 해당 메시지를 어딘가에 보관하고 있어야 한다.
